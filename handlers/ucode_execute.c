...
static EFI_STATUS check_job_parameters(JobParameters* job_parameters, ConnectionContext* ctx) {
    ...
    if (context->present != 1) {
        ...
        send_status(0x106, FormatBuffer, ctx);
        return EFI_INVALID_PARAMETER;
    }

    if (context->started != 1) {
        FormatPrint(L"Core id %u is not started.\n", job_parameters->core_id);
        send_status(0x107, FormatBuffer, ctx);
        return EFI_INVALID_PARAMETER;
    }
...
static EFI_STATUS acquire_core_lock(JobParameters* job_parameters, ConnectionContext* ctx) {
    CoreContext* context = &core_contexts[job_parameters->core_id];
...
static void prepare_ucode_update(CoreContext* context) {
...
static void write_job_infos(JobParameters* job_parameters, CoreContext* context) {
    ...
    context->prepare_function = prepare_ucode_update;
    context->prepare_function_argument = context;

    if ((job_parameters->options & 0x01) == 0x01) {
        context->job_function = apply_ucode_execute_machine_code_restore;
    } else {
        context->job_function = apply_ucode_execute_machine_code_simple;
    }
...
EFI_STATUS handle_apply_ucode_execute_test(UINT8* payload, UINTN payload_length, ConnectionContext* ctx) {
    ...
    // at this point the requested core is in theory available for scheduling a job
    // try to get a lock on it's context
    // if this fails the core is still busy or hung
    // or we are in a weird state and should not use the core anyway
    status = acquire_core_lock(job_parameters, ctx);
    if (status != EFI_SUCCESS) {
        return status;
    }
    ...
    if (job_parameters->core_id == 0) {
        ...
        JobFunction job = context->job_function;
        job(context->job_function_argument);
        context->ready = 1;
